...
export default class Tasks {
  ...
  drawUi() {
    const checkedProject = this.projects.find((project: Project) => project.isCheck === true);
    ...
    if (projectsListElement) {
      fromEvent(projectsListElement, 'click').subscribe(() => this.showProjectsList());
    }

    this.container.appendChild(tasksEl);
    checkedProject?.tasks.forEach((task : Task) => this.addTask(checkedProject.id, task));
  }
    ...
    tasksEl.innerHTML = `
      ...
      <div class="tasks">
        <table>
          <thead>
            <tr>
              <th class="projects-list" column="3" data-id="${checkedProject?.id}">Project: <span class="choosen-project">${checkedProject?.name}</span></th>
              <th ></th>
            </tr>
          </thead>
